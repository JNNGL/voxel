...
static size_t ramdiskfs_read(fs_node_t* node, size_t offset, size_t size, uint8_t* buffer) {
    uintptr_t base = (uintptr_t) ((fs_node_t*) node->device)->context;
    ramdiskfs_header_t* header = (ramdiskfs_header_t*) base;
    if (header->signature != RDFS_SIGNATURE) {
        return 0;
    }

    uint32_t current_entry = node->device == node ? header->root_entry : node->context;
    ...
    memcpy(buffer, (void*) (base + offset + entry->offset + sizeof(ramdiskfs_entry_t)), size);
    return size;
}

static struct dirent* ramdiskfs_readdir(fs_node_t* node, size_t index) {
    uintptr_t base = (uintptr_t) ((fs_node_t*) node->device)->context;
    ramdiskfs_header_t* header = (ramdiskfs_header_t*) base;
    if (header->signature != RDFS_SIGNATURE) {
        return 0;
...
static struct fs_node* ramdiskfs_finddir(fs_node_t* node, char* name) {
    ...
    while (current_entry) {
        ...
        if (strcmp(entry->name, name) == 0) {
            fs_node_t* rnode = malloc(sizeof(fs_node_t));
